...
class LatentConsistencyModelPipeline(
    ...
    def __call__(
        ...
        height: Optional[int] = 768, # maybe change to 512
        width: Optional[int] = 768, # maybe change to 512
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents, denoised, log_prob = self.scheduler.step(model_pred, t, latents, **extra_step_kwargs, return_dict=False)
        ...
        #     img = img.to(prompt_embeds.dtype)
        #     img = self.vae.decode(img / self.vae.config.scaling_factor, return_dict=False)[0]
        #     img = self.image_processor.postprocess(
        #         img, output_type=output_type, do_denormalize=[True] * img.shape[0]
        #     )
        ...
        if not return_dict:
            return (image, states, log_probs)
        
        states = torch.stack(states, dim=1)  # (batch_size, num_steps + 1, 4, 64, 64)
        ...
        return {
            ...
            "log_probs": log_probs,
            "prompt_embeds": prompt_embeds,
            "time_index": torch.arange(0, len(timesteps))
            .repeat(states.shape[0], 1)
            .to(dtype=int),  # (batch size, num_steps)
            "timesteps": timesteps.repeat(states.shape[0], 1).to(
                device
            ),  # (batch size, num_steps)
        }